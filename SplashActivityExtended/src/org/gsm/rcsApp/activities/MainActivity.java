...
public class MainActivity extends Activity implements Runnable {
						...
						try {
							...
							if (errorCode == 200) {
								...
								if (contactCollection != null) {
									JSONArray contactList = Utils.getJSONArray(
											contactCollection, "contact");
									retrievedContacts.clear();
									if (contactList != null) {
										for (int i = 0; i < contactList
												.length(); i++) {
											JSONObject contact = (JSONObject) contactList
													.get(i);
											...
											JSONObject attributeList = Utils
											...
											if (attributeList != null) {
												resourceURL = Utils
														.getJSONStringElement(
																attributeList,
																"resourceURL");
												JSONArray attributes = Utils
														.getJSONArray(
																attributeList,
																"attribute");
												if (attributes != null) {
													for (int a = 0; a < attributes
															.length(); a++) {
														JSONObject attribute = attributes
																.getJSONObject(a);
														String name = Utils
																.getJSONStringElement(
																		attribute,
											...
											// String
											// icon=contact.getString("icon");
											// String
											// status=contact.getString("status");
											...
											contactRecord.setHasNewMessage(cs
													.isNewMessage());

											retrievedContacts
													.add(contactRecord);
											contactMap.put(contactId,
													contactRecord);
										}
	...
	public void run() {
		...
		while (running) {
			...
			makeRequest = (now - lastRequestTime) >= MAIN_LOOP_DELAY; // Throttle
																		// so
																		// that
																		// do
																		// not
			...
			final String notificationsUrl = SplashActivity.notificationChannelURL != null ? (SplashActivity.notificationChannelURL
					.replaceAll("tel\\:\\+", "tel%3A%2B").replaceAll("tel\\:",
					"tel%3A")) : null;
			...
			if (notificationsUrl != null && makeRequest) {
				...
				int statusCode = response != null
						&& response.getStatusLine() != null ? response
	...
	private void processNotificationResponse(int statusCode, JSONObject response)
		...
		JSONArray notifications = response != null ? response
				.getJSONArray("notificationList") : null;
		if (notifications != null && notifications.length() > 0) {
			for (int i = 0; i < notifications.length(); i++) {
				JSONObject notification = notifications.getJSONObject(i);

				Log.d("MainActivity", "Notification [" + i + "] = "
						+ notification.toString());

				if (Utils.getJSONObject(notification, "messageNotification") != null) {
					JSONObject messageNotification = Utils.getJSONObject(
							notification, "messageNotification");

					if (Utils.getJSONObject(messageNotification, "isComposing") != null) {
						JSONObject isComposing = Utils.getJSONObject(
								messageNotification, "isComposing");
						String senderAddress = Utils.getJSONStringElement(
								messageNotification, "senderAddress");
						String status = Utils.getJSONStringElement(isComposing,
								"status");
						...
						if (senderAddress != null) {
							if (senderAddress
								...
								ChatSessionActivity
										.updateComposingIndicator(status);
							}
							contactStateManager.updateComposingIndicator(
									senderAddress, status);
						}
					}

					if (Utils.getJSONObject(messageNotification, "chatMessage") != null) {
						JSONObject chatMessage = Utils.getJSONObject(
								messageNotification, "chatMessage");
						String senderAddress = Utils.getJSONStringElement(
								messageNotification, "senderAddress");
						String messageId = Utils.getJSONStringElement(
								messageNotification, "messageId");
						String sessionId = Utils.getJSONStringElement(
								messageNotification, "sessionId");
						String messageText = Utils.getJSONStringElement(
								chatMessage, "text");
						String dateTime = Utils.getJSONStringElement(
								messageNotification, "dateTime");

						ArrayList<ChatMessage> messageBuffer = messageCache
								.get(senderAddress);
						...
						received.setMessageTime(Utils
								.convertTransferDateToDisplayString(dateTime));
						received.setStatus(ChatMessage.MESSAGE_STATUS_RECEIVED);

						String url = ServiceURL.getSendMessageStatusReportURL(
								SplashActivity.userId, senderAddress,
								sessionId, messageId);
						...
						if (senderAddress != null) {
							if (senderAddress
								...
								ChatSessionActivity.refreshMessageList(
										senderAddress, SplashActivity.userId,
										received);
							} else {
						...
						AuthScope authscope = new AuthScope(
								ServiceURL.serverName, ServiceURL.serverPort,
								AuthScope.ANY_REALM);
						responseClient
								.setBasicAuth(SplashActivity.userId,
										SplashActivity.appCredentialPassword,
										authscope);

						final String responseURL = url;
						Log.d("MainActivity", "Sending message status update "
								+ responseURL + " with " + jsonData);

						StringEntity requestData;
						try {
							requestData = new StringEntity(jsonData);
							responseClient.put(_instance.getApplication()
									.getApplicationContext(), url, requestData,
									"application/json",
					...
					if (link != null && link.length() > 0) {
						for (int li = 0; li < link.length(); li++) {
							...
							if ("ParticipantSessionStatus".equals(rel)
								...
								AuthScope authscope = new AuthScope(
										ServiceURL.serverName,
										ServiceURL.serverPort,
										AuthScope.ANY_REALM);
								acceptClient.setBasicAuth(
